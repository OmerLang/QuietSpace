...
export async function isTypePlace(placeId) {
  ...
  const { data, error } = await staticSupabase.rpc("get_poi_by_placeid", {
    placeid: placeId,
  });
  if (error) {
  ...
  if (data === null || data.length === 0) {
    ...
    const response = await fetch(
      `https://places.googleapis.com/v1/places/${placeId}?languageCode=he&regionCode=IL`,
      {
        method: "GET",
        headers: {
          ...
          "X-Goog-FieldMask":
            "id,types,location,addressComponents,displayName,outdoorSeating,photos",
        },
      },
    );
