...
export default function SignupForm({ setIsSignupWelcomeOpen }) {
  ...
  const signup = async (data) => {
    ...
    setTimeout(() => openOverlay("login"), 5200);
    return;
  };
  ...
  return (
    <form onSubmit={handleSubmit(signup)} className={styles.signupForm}>
      <div className={styles.fieldDiv}>
        ...
        <div
          className={`${styles.errorWrapper} ${errors.fullName ? styles.errorWrapperShow : ""}`}
        >
      ...
      <div className={styles.fieldDiv}>
        ...
        <div
          className={`${styles.errorWrapper} ${errors.confirmPassword ? styles.errorWrapperShow : ""}`}
        >
      ...
      >
        <span>Already have an account? click to login</span>
      </button>
      <button
        type="submit"
      ...
      >
        <span className={styles.submitBtnText}>
          {isSubmitting ? <LoadingSpinner size={30} /> : "Signup"}
        </span>
      </button>
    </form>
  );
